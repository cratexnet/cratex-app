# <img src="favicon.png" alt="" width="32" height="32" align="absmiddle"> CrateX.app

## 🔗 빠른 내비게이션

- [온라인에서 모든 도구 사용](https://cratex.app/ko)
- [CrateX.app에 대해 알아보기](https://cratex.app/ko/about)
- [요금제 보기](https://cratex.app/ko/pricing)
- [개인정보 처리방침](https://cratex.app/ko/privacy)
- [쿠키 정책](https://cratex.app/ko/cookie-policy)
- [이용 약관](https://cratex.app/ko/terms)

## 🌍 언어

[English](README.md) | [简体中文](README_zh-CN.md) | [正體中文](README_zh-TW.md) | [日本語](README_ja.md) | 한국어 | [ไทย](README_th.md) | [Bahasa Indonesia](README_id.md) | [Tiếng Việt](README_vi.md) | [हिन्दी](README_hi.md) | [Français](README_fr.md) | [Deutsch](README_de.md) | [Español](README_es.md) | [Italiano](README_it.md) | [Nederlands](README_nl.md) | [Polski](README_pl.md) | [Português](README_pt.md) | [Русский](README_ru.md) | [Türkçe](README_tr.md) | [العربية](README_ar.md)

## CrateX.app 소개

CrateX.app은 개발자, 디자이너, 크리에이터와 일상적인 워크플로를 위한 빠른 브라우저 기반 도구 플랫폼입니다. 이 저장소는 CrateX.app의 공식 공개 피드백 및 커뮤니티 허브이며, 애플리케이션 소스 코드 저장소가 아닙니다.

### 저장소 범위

- 이 저장소는 버그 보고, 기능 요청, 번역 제안, 로드맵 피드백, 커뮤니티 논의에 사용해 주세요.
- CrateX.app의 프로덕션 소스 코드는 이 저장소에 공개되지 않습니다.
- CrateX.app의 이름, 로고, 브랜드 자산은 혼동을 일으킬 수 있는 제3자 제품 운영에 사용할 수 있도록 라이선스되지 않습니다.

### 🚀 안내사항

웹사이트는 아직 빠르게 발전하고 있으며, 새로운 도구가 지속적으로 출시되고 기존 카테고리와 인터페이스가 조정될 수 있습니다. 저희는 현대적이고 친숙하며 사용하기 쉬운 도구 사이트 구축에 최선을 다하고 있습니다. 여러분의 이해와 지원에 감사드립니다!

### ✨ 주요 기능

- 🔒 대부분의 도구는 기본적으로 로컬 처리
- ⚡ 대부분의 도구는 브라우저에서 빠르게 처리하고 즉시 결과를 제공합니다
- 🌍 다국어 인터페이스 지원
- 🆓 대부분의 도구는 계정 없이 사용할 수 있습니다
- 🔐 유료 계정은 자체 저장소로 암호화된 WebDAV Sync를 사용할 수 있습니다
- 📱 휴대폰, 태블릿, 데스크톱에 맞게 조정됩니다
- 🧩 PWA(Progressive Web App)로 설치할 수 있어 자주 사용하는 도구에 더 빠르게 접근할 수 있습니다 ([PWA란?](https://developer.mozilla.org/ko/docs/Web/Progressive_web_apps))

### 개인정보, 계정, 동기화 범위

- 대부분의 도구 입력과 결과는 브라우저에서 로컬로 처리됩니다. 로그인, 결제, 유료 혜택, WebDAV Sync, 통계, 사이트 보안에는 필요한 서버 기능만 사용합니다.
- 대부분의 도구는 로그인 없이 사용할 수 있습니다. 로그인은 주로 유료 혜택 복원, 구독 관리, WebDAV Sync 사용을 위해 필요합니다.
- 유료 계정은 입력 길이, 파일 크기, 일괄 처리, 복잡한 미리보기의 더 높은 한도와 광고 없는 경험을 받을 수 있습니다.
- WebDAV Sync는 사용자의 자체 저장소에 기록합니다. 복구 키를 안전하게 보관하세요. 키를 잃으면 CrateX.app은 원격 암호화 동기화 데이터를 복구할 수 없습니다.
- 기본적으로 언어, 테마, 즐겨찾기, 최근 사용 도구, 도구 설정, 감사된 소량의 사용 데이터를 동기화합니다. API 키, 업로드한 파일, 현재 페이지의 임시 입력, 생성된 결과는 동기화하지 않습니다.

### 🛠️ 도구 카테고리 (지속적으로 추가/개선 중)

#### 🛠️ [유틸리티 도구](https://cratex.app/ko/cat/utils)

- [비밀번호 생성기](https://cratex.app/ko/password-generator)
- [해시 생성 및 검증](https://cratex.app/ko/hash-checksum)
- [UUID 생성기](https://cratex.app/ko/uuid-generator)
- [QR 코드 생성기](https://cratex.app/ko/qr-generator)
- [QR 디코더](https://cratex.app/ko/qr-decoder)
- [바코드 인식](https://cratex.app/ko/barcode-decoder)
- [백분율 계산기](https://cratex.app/ko/percentage-calculator)
- [단위 변환기](https://cratex.app/ko/unit-converter)
- [나이 계산기](https://cratex.app/ko/age-calculator)
- [날짜 계산기](https://cratex.app/ko/date-calculator)
- [시간 계산기](https://cratex.app/ko/time-duration-calculator)
- [타임스탬프 변환기](https://cratex.app/ko/timestamp-converter)
- [세계 시계](https://cratex.app/ko/world-clock)
- [난수 생성기](https://cratex.app/ko/random-number-generator)
- [무작위 추첨](https://cratex.app/ko/random-picker)
- [주사위 굴리기](https://cratex.app/ko/dice-roller)
- [동전 던지기](https://cratex.app/ko/coin-flip)

#### 📝 [텍스트 처리](https://cratex.app/ko/cat/text)

- [텍스트 카운터](https://cratex.app/ko/text-counter)
- [스마트 텍스트 클리너](https://cratex.app/ko/text-formatter)
- [Unicode 문자 정리](https://cratex.app/ko/unicode-cleaner)
- [목록 정리](https://cratex.app/ko/list-cleaner)
- [텍스트 찾기 및 바꾸기](https://cratex.app/ko/find-replace)
- [텍스트 정보 추출](https://cratex.app/ko/text-pattern-extractor)
- [텍스트 차이 뷰어](https://cratex.app/ko/text-diff)
- [대소문자 변환](https://cratex.app/ko/case-converter)
- [슬러그 생성기](https://cratex.app/ko/slug-generator)
- [Markdown 변환기](https://cratex.app/ko/markdown-converter)

#### 🖼️ [이미지 도구](https://cratex.app/ko/cat/image)

- [이미지 형식 변환기](https://cratex.app/ko/image-converter)
- [이미지 텍스트 추출](https://cratex.app/ko/image-to-text)
- [이미지 크기 조정](https://cratex.app/ko/image-resizer)
- [이미지 압축](https://cratex.app/ko/image-compressor)
- [이미지 자르기](https://cratex.app/ko/image-cropper)
- [이미지 워터마크 추가](https://cratex.app/ko/image-watermarker)
- [이미지 합치기](https://cratex.app/ko/image-merger)
- [긴 스크린샷 이어붙이기](https://cratex.app/ko/long-screenshot-stitcher)
- [이미지 컬러 피커와 팔레트](https://cratex.app/ko/image-color-picker)
- [이미지 메타데이터 보기 및 정리](https://cratex.app/ko/image-metadata-cleaner)
- [이미지 개인정보 가리기](https://cratex.app/ko/image-redactor)

#### 📄 [PDF 도구](https://cratex.app/ko/cat/pdf)

- [이미지를 PDF로 변환](https://cratex.app/ko/image-to-pdf)
- [PDF 이미지 변환](https://cratex.app/ko/pdf-to-images)
- [PDF 텍스트 변환](https://cratex.app/ko/pdf-to-text)

#### 🎬 [미디어 도구](https://cratex.app/ko/cat/media)

- [동영상에서 오디오 추출](https://cratex.app/ko/extract-audio-from-video)
- [동영상 음소거](https://cratex.app/ko/remove-audio-from-video)
- [오디오 자르기](https://cratex.app/ko/trim-audio)
- [오디오를 WAV로 변환](https://cratex.app/ko/audio-to-wav)
- [동영상 프레임 추출](https://cratex.app/ko/video-frame-extractor)
- [M4A를 MP3로 변환](https://cratex.app/ko/m4a-to-mp3)

#### 🔄 [인코딩/변환](https://cratex.app/ko/cat/encoding)

- [URL 인코딩/디코딩](https://cratex.app/ko/url-encoder)
- [URL 파서](https://cratex.app/ko/url-parser)
- [Base64 인코딩/디코딩](https://cratex.app/ko/base64-encoder)
- [JSON 이스케이프/언이스케이프](https://cratex.app/ko/json-escape)
- [HTML 이스케이프/언이스케이프](https://cratex.app/ko/html-encoder)
- [Unicode 변환기](https://cratex.app/ko/unicode-converter)
- [Unicode 정규화](https://cratex.app/ko/unicode-normalizer)
- [ASCII 변환기](https://cratex.app/ko/ascii-converter)
- [진법 변환기](https://cratex.app/ko/base-converter)

#### 💻 [개발 도구](https://cratex.app/ko/cat/dev)

- [JSON 포맷 및 검증](https://cratex.app/ko/json-formatter)
- [JSON 복구 및 검사](https://cratex.app/ko/json-repair)
- [JSON을 TypeScript로](https://cratex.app/ko/json-to-typescript)
- [JSON YAML 변환](https://cratex.app/ko/yaml-json-converter)
- [JSON ↔ CSV 변환](https://cratex.app/ko/csv-json-converter)
- [JWT 디코더](https://cratex.app/ko/jwt-decoder)
- [정규식 테스트](https://cratex.app/ko/regex-tester)
- [MIME 타입](https://cratex.app/ko/mime-type)
- [HTTP 상태 코드](https://cratex.app/ko/http-status)
- [CSS 단위 변환기](https://cratex.app/ko/css-unit-converter)
- [색상 변환기](https://cratex.app/ko/color-converter)
- [Lorem 생성기](https://cratex.app/ko/lorem-generator)
- [Cron 표현식 파서](https://cratex.app/ko/cron-parser)

## 🐛 문제 신고

사용 중 문제가 발생하거나 개선 제안이 있으시면 다음 방법으로 피드백을 주시기 바랍니다:

### GitHub Issues (권장)

👉 [GitHub Issues](https://github.com/cratexnet/cratex-app/issues/new/choose)

### 피드백 형식

문제를 더 잘 해결할 수 있도록 다음 형식에 따라 피드백을 제출해 주세요:

#### 🐛 버그 신고 템플릿

```
문제 설명:
- 발생한 문제를 간단히 설명

재현 단계:
1. 관련 도구 페이지 열기
2. 테스트 데이터 입력 또는 업로드
3. 해당 작업 실행
4. 문제 확인

예상 결과:
- 예상하는 올바른 결과 설명

실제 결과:
- 실제로 발생한 상황 설명

환경 정보:
- 브라우저: Chrome/Firefox/Safari 버전
- 운영체제: Windows/Mac/iOS/Android
- 기기: 데스크톱/모바일/태블릿
- 언어 인터페이스: 한국어/영어 등

스크린샷 (선택사항):
- 문제를 더 잘 설명할 수 있는 스크린샷이 있으면 업로드
```

#### 💡 기능 요청 템플릿

```
제안하는 기능 또는 도구:
- 추가를 원하는 도구나 기능, 또는 개선을 원하는 기존 경험을 간단히 설명

사용 사례:
- 언제 이 기능을 사용할지

구체적 요구사항:
- 기능이 어떻게 작동해야 하는지 자세한 설명

우선순위:
- 높음/중간/낮음
```

#### 🌐 번역 문제 템플릿

```
언어:
- 번역 문제가 있는 언어

위치:
- 어떤 페이지/도구에서 문제를 발견했는지

현재 번역:
- 현재 표시되는 텍스트

제안 번역:
- 더 적절하다고 생각하는 번역

설명 (선택사항):
- 왜 그 번역이 더 좋은지
```

### 📧 이메일 연락

GitHub를 사용하기 불편하시면 [support@cratex.app](mailto:support@cratex.app) 으로 이메일을 보내 주세요. 더 빠른 응답과 원활한 소통을 위해 GitHub Issues 사용을 권장합니다.

이메일에는 문제 유형을 명시하고 자세한 정보를 제공해 주세요.

## 💬 커뮤니티

추가되었으면 하는 도구, 아직 사용하기 불편한 기존 도구, 더 단순해질 수 있는 일상 작업 흐름의 작은 필요가 있다면 커뮤니티에서 알려 주세요.

- [Discord에 참여](https://discord.gg/JWhE4TKsBN)해 커뮤니티 토론, 간단한 질문, 제품 업데이트를 확인하세요.
- [X에서 CrateX.app 팔로우](https://x.com/cratexnet)로 릴리스 노트와 공지를 받아보세요.

## 🤝 저희의 약속

- 💨 여러분의 피드백을 최대한 빨리 검토하고 답변하겠습니다
- 🔄 사용자 피드백을 바탕으로 도구 경험을 지속적으로 최적화
- 👂 모든 제안이 저희 개선의 동력입니다

## 🌟 미래 발전

우수한 도구 플랫폼에는 지속적인 진보와 개선이 필요하다는 것을 깊이 이해하고 있습니다. CrateX.app 은 항상:

- 실제 사용 사례에서 반복적으로 나타나는 요구를 우선적으로 살펴보기
- 기존 도구의 기능과 사용자 경험을 지속적으로 개선
- 사용자 요구와 기술 발전 트렌드에 따라 도구 라이브러리를 지속적으로 확충
- 전 세계 사용자에게 더 나은 현지화 경험 제공

❤️
